...
namespace CoreRPC.Serialization
{
    public class JsonMethodCallSerializer : IMethodCallSerializer
    {
        ...
        public JsonMethodCallSerializer(JsonSerializer serializer, bool _ = false)
        {
            _serializer = serializer;
        }

        public JsonMethodCallSerializer(bool _ = false) : this(new JsonSerializer())
        {
        ...
        JsonWriter CreateWriter(Stream stream) => new JsonTextWriter(new StreamWriter(stream, Utf8, 1024, true));

        JsonReader CreateReader(Stream stream) => new JsonTextReader(new StreamReader(stream));
        ...
        public void SerializeCall(Stream stream, IMethodBinder binder, string target, MethodCall call)
        {
            ...
            using (var writer = CreateWriter(stream))
            {
                ...
                writer.WriteValue(Convert.ToBase64String(sig));
